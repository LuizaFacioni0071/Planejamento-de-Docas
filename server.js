...
function processLocalSheetData() {
    try {
        ...
        if (!fs.existsSync(filePath)) {
            return console.error(`ERRO: Ficheiro da planilha não encontrado em: ${filePath}`);
        }
...
app.get('/api/gerar-relatorio', (req, res) => {
    try {
        ...
        for (const monthYear in tasksByMonth) {
            tasksByMonth[monthYear].sort((a, b) => new Date(a.Data.split('/').reverse().join('-')) - new Date(b.Data.split('/').reverse().join('-')));
        }
